...
public class CertificateRevocationStatus {
  ...
  public static void fetchStatus(BigInteger serialNumber, ThreadUtils.Callback<CertificateRevocationStatus> callback)
       {
//           try {
//               CertificateRevocationStatus certificateRevocationStatus = fetchStatus(serialNumber.toString(16));
//             callback.onSuccess(certificateRevocationStatus);
//           } catch (IOException e) {
//               callback.onError(e);
//           }
           fetchStatus(serialNumber.toString(16), new ThreadUtils.Callback<CertificateRevocationStatus>() {
             @Override
  ...
  public static void fetchStatus(String serialNumber, ThreadUtils.Callback<CertificateRevocationStatus> callback) {
    URL url;
    try {
    ...
    call.enqueue(new Callback() {
      public void onResponse(Call call, Response response)
              throws IOException {
        callback.onSuccess(decodeStatus(serialNumber, response.body().charStream()));
      }
